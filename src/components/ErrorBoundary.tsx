    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex h-screen w-screen flex-col items-center justify-center gap-4 bg-[var(--bg-app)] p-8">
          <div className="rounded-lg border border-red-500/30 bg-red-950/20 p-6 max-w-lg w-full">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => this.setState({ support: true })} className="rounded border border-white/10 px-4 py-2 text-sm">Contact Support</button>
              <button
              ...
              {this.state.error?.stack ? (
                <button
                  ...
                  className="rounded border border-white/10 px-4 py-2 text-sm font-medium text-zinc-300 hover:bg-white/5"
                >
                  Copy details
                </button>
              ) : null}
