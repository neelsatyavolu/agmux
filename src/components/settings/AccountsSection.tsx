...
export function AccountsSection() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const result = await providerAccounts.list();
    ...
    <section className="space-y-6 text-[var(--text-primary)]" aria-label="Provider accounts">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className={button} disabled={locked} onClick={() => void reload()} aria-label="Refresh accounts" title="Refresh accounts">
              ...
              {rows.map(account => {
                ...
                return <article key={account.id} aria-label={account.label} className="p-4">
                  ...
                  <div className="mt-2 flex flex-wrap items-center justify-between gap-1 text-xs text-[var(--text-tertiary)]">
                    <span className="tabular-nums">{remaining === null ? "Usage unavailable" : <><span className="font-medium text-[var(--text-secondary)]">{Math.round(remaining)}%</span> left</>}</span>
                  ...
                  {account.native && <div className="mt-3 flex flex-wrap items-center gap-2">
                    <button className={button} disabled={locked} onClick={() => void run(() => providerAccounts.refresh(account.id, null))}>Check usage</button>
                    {account.provider !== "claude" && <button className={button} disabled={locked} onClick={() => void run(() => providerAccounts.importCurrent({ provider: account.provider, label: account.label, teamId: null }), "Account added to switching.")}>Add to switching</button>}
                  </div>}
                  {canEdit && <>
