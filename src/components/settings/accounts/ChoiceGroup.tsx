...
export function ChoiceGroup<T extends string>({ label, choices, value, onChange, disabled = false }: {
  ...
  return (
    <div role="radiogroup" aria-label={label} onKeyDown={onKeyDown} className="inline-flex max-w-full flex-wrap gap-1 rounded-lg border border-[var(--glass-border)] p-1">
      {choices.map(choice => {
        ...
        return <button key={choice.value} type="button" role="radio" aria-checked={active} data-value={choice.value}
          ...
          className={`min-h-8 rounded-md px-3 text-xs font-medium transition-colors focus-visible:outline-2 focus-visible:outline-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-40 ${active
