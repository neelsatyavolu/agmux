-- Add 'Pi' to the threads.provider CHECK constraint (pi-coding-agent CLI).
-- SQLite requires table recreation to modify CHECK constraints.
-- Child tables with ON DELETE CASCADE FKs must be backed up first (same
-- pattern as 027_kimi_provider.sql). agent_room_members was added in 028.

CREATE TEMP TABLE xanom_034_agent_logs_backup AS
SELECT id, thread_id, direction, content, timestamp, log_type
FROM agent_logs;

CREATE TEMP TABLE xanom_034_thread_journal_entries_backup AS
SELECT
    id, thread_id, kind, title, content, source, confidence, created_by,
    created_at, updated_at, is_archived
FROM thread_journal_entries;

CREATE TEMP TABLE xanom_034_prompt_logs_backup AS
SELECT
...
CREATE TEMP TABLE xanom_034_thread_turns_backup AS
SELECT
...
CREATE TEMP TABLE xanom_034_agent_room_members_backup AS
...
-- Skip child rows whose thread was already deleted (FK ON would abort
-- startup). One live DB had leftover thread_turns for zz-verify-thread-b.
INSERT INTO agent_logs (id, thread_id, direction, content, timestamp, log_type)
SELECT id, thread_id, direction, content, timestamp, log_type
FROM xanom_034_agent_logs_backup
...
SELECT
    id, thread_id, kind, title, content, source, confidence, created_by,
    created_at, updated_at, is_archived
FROM xanom_034_thread_journal_entries_backup
WHERE thread_id IN (SELECT id FROM threads);
...
SELECT
    ...
    context_fetched, context_score, context_reason, context_mode,
    final_prompt_sent, timestamp
FROM xanom_034_prompt_logs_backup
WHERE thread_id IN (SELECT id FROM threads);
...
SELECT
    ...
    summary, summary_source, anchor_kind, anchor_ref, facts_json, created_at,
    prompt_summary
FROM xanom_034_thread_turns_backup
WHERE thread_id IN (SELECT id FROM threads);

INSERT INTO agent_room_members (room_id, thread_id, label, sort_order)
SELECT room_id, thread_id, label, sort_order
FROM xanom_034_agent_room_members_backup
WHERE thread_id IN (SELECT id FROM threads);

DROP TABLE xanom_034_agent_logs_backup;
DROP TABLE xanom_034_thread_journal_entries_backup;
DROP TABLE xanom_034_prompt_logs_backup;
DROP TABLE xanom_034_thread_turns_backup;
DROP TABLE xanom_034_agent_room_members_backup;
