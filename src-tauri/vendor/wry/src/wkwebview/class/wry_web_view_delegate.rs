...
use objc2_foundation::{ns_string, MainThreadMarker, NSObjectProtocol, NSString, NSURLRequest};
use objc2_web_kit::{
  WKFrameInfo, WKScriptMessage, WKScriptMessageHandler, WKUserContentController,
};
    ...
    fn did_receive(
      ...
      unsafe {
        ...
        if let Ok(body) = body.downcast::<NSString>() {
          ...
          let frame_info: Option<Retained<WKFrameInfo>> = msg_send![msg, frameInfo];
          let Some(frame_info) = frame_info else {
            #[cfg(feature = "tracing")]
            tracing::warn!("WebView received IPC with no frame info.");
            return;
          };
          let request: Option<Retained<NSURLRequest>> = msg_send![&*frame_info, request];
          let Some(request) = request else {
            #[cfg(feature = "tracing")]
            tracing::warn!("WebView received IPC with no request.");
            return;
          };
          // Same macOS-beta nil path as custom-protocol: JS IPC must not abort
          // the process when WebKit omits request.URL / absoluteString.
          let Some(url) = request.URL() else {
            #[cfg(feature = "tracing")]
            tracing::warn!("WebView received IPC with no request URL.");
            return;
          };
          let Some(absolute_url) = url.absoluteString() else {
            #[cfg(feature = "tracing")]
            tracing::warn!("WebView received IPC with no absolute URL.");
            return;
          };
