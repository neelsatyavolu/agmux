...
extern "C" fn start_task(
  ...
  unsafe {
    ...
    if let Some(function) = function {
      // Get url request.
      // Never unwrap NSURLRequest / NSURL / NSString here: on newer macOS
      // (26/27 beta) WebKit can hand a WKURLSchemeTask whose `request` is
      // nil even though objc2 declares it non-null. That panics on the
      // AppKit thread (keystroke IPC, JS loads) and abort-kills the app.
      let request: Option<Retained<NSURLRequest>> = msg_send![task, request];
      ...
      let Some(uri) = url.absoluteString() else {
        return;
      };
      ...
      if let Some(body) = body {
        ...
        while body_stream.hasBytesAvailable() {
          ...
          let len = sent_form_body.len();
          let read_length = sent_form_body.capacity() - len;
          if read_length == 0 {
            break;
          }
          let Some(p) = NonNull::new(sent_form_body.as_mut_ptr().add(len)) else {
            break;
          };
          // NSInputStream read:maxLength: returns -1 on error. Casting that
          // to usize and set_len would abort.
          let count = body_stream.read_maxLength(p, read_length);
          if count <= 0 {
            break;
          }
          sent_form_body.set_len(len + count as usize);
        }
      ...
      let respond_with_404 = || {
        ...
        let Ok(status) = StatusCode::NOT_FOUND.as_u16().try_into() else {
          return;
        };
          ...
          let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
            Box::new(move |sent_response| {
              ...
              unsafe fn response(
                ...
                if let Some(mime) = wanted_mime.and_then(|m| m.to_str().ok()) {
                  headers.insert(
                ...
                let urlresponse = NSHTTPURLResponse::alloc();
                let status = wanted_status_code.try_into().unwrap_or(200);
                let Some(response) = NSHTTPURLResponse::initWithURL_statusCode_HTTPVersion_headerFields(
                  urlresponse,
                  &url,
                  status,
