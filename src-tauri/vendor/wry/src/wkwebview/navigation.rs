use objc2::{msg_send, rc::Retained, DeclaredClass};
use objc2_foundation::{NSObjectProtocol, NSString, NSURLRequest};
use objc2_web_kit::{
...
pub(crate) fn navigation_policy(
  ...
  unsafe {
    ...
    let request: Option<Retained<NSURLRequest>> = msg_send![action, request];
    let Some(url) = request.and_then(|r| r.URL()).and_then(|u| u.absoluteString()) else {
