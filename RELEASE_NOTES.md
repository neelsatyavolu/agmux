...
- **Teams session counts** — Teams, and Settings → Your data, now show how many sessions each person actually started. Before, one long session was counted again for every hour it ran, and automatic reviews and helper agents counted as sessions too. Their usage still counts toward tokens, cost and active time.
...
- **Teams date ranges** — The 7, 14, 30 and 90 day views no longer count an extra day in their totals that the daily chart doesn't show, so totals, active days and the comparison with the previous period cover exactly the days you picked.
- **Teams peak sessions** — "Peak simultaneous sessions" now counts agents you run at the same time in different projects or with different agents. Before, it only counted sessions in the same project, so running several at once often showed as 1.
- **Teams tool counts for Claude** — When Claude ran several tools at once, Teams counted only the last one, so tool calls, edits, changed lines and the error rate were too low. Every tool call is now counted.
- **Teams code changes for Codex** — Files changed and lines added or removed by current Codex versions now show up in Teams. Before, Codex edits were reported as no changes at all.
- **Teams token composition** — Reasoning tokens are now shown as part of output instead of being counted a second time, so the token composition bar and its percentages add up to the token total.
- **Teams history is kept when uploads pause** — When a team's trial ends and its dashboards become read-only, or when someone leaves a team, that team's past activity now stays visible instead of disappearing on the member's next sync.
