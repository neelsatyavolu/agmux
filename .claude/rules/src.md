...
## Sidebar Focus group
- Opt-in (`settings.focusEnabled`, `focusWindowHours`). `FocusSection` owns only the header, the `[data-focus-list]` container and the project picker; every `ProjectGroup` portals its own qualifying rows into that container (`focusPortal`/`focusSince` props) through the shared `renderItem(item, inFocus)`, so Focus rows keep list behavior. Rows qualify by `unified` timestamp inside the window, or while working/awaiting approval/unread.
