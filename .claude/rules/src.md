...
- Idle cost: a new infinite CSS animation must be covered by the `html.app-backgrounded` pause list in `index.css` or carry `.bg-pausable` (only infinite ones — entrance/exit animations must keep running). Presentation-only polls (usage, timeline rebind, discovery, elapsed clocks) use `syncPollingToAppForeground` and refresh on resume; the Claude SDK scroll pin deliberately keeps following while the window is visible but unfocused.
...
## Sidebar Focus group
- Opt-in (`settings.focusEnabled`, `focusWindowMinutes` 5–30, default 10). `FocusSection` owns only the header, the `[data-focus-list]` container, Show more/less, the header right-click "Threads visible" stepper (`focusThreadsVisible`, default 7) and the project picker; every `ProjectGroup` portals its own qualifying rows into that container (`focusPortal`/`focusSince`/`focusCutoff` props) through the shared `renderItem(item, inFocus)`, so Focus rows keep list behavior. Rows qualify by `unified` timestamp inside the window, or while working/awaiting approval (unread alone does not keep a row).
- Row cap across projects: each group publishes its qualifying row times to `focusRowsStore` (layout effect, so no frame shows extra rows); Sidebar computes `focusCutoff(times, limit + extraShown)` and groups portal only rows at or after it. The header count is the store total, not the DOM.
- `FocusIntroDialog` (mounted in `App.tsx`) offers Focus once (`focusIntroSeen`). It waits for the setup wizard, What's New (`isWhatsNewPending`) and the local-model offer. Its preview reuses the real sidebar classes plus `ProviderIcon`/`StatusDot` exported from `ProjectGroup` — keep it in step with real Focus rows.
...
- Visible names (2026-09-23): tab `agentAccounts` is labelled **Accounts**; tab `accounts` (Git identities, connections) is labelled **Git & Connections**. Tab IDs are unchanged so deep links keep working.
- Native CLI logins appear automatically as read-only `native` rows with `currentLogin`. They are already first in automatic switching, so the UI has no "Add to switching"/"Use existing login" copy action (user decision 2026-09-23); never auto-copy native credentials. A managed row with the same verified identity gets the badge without duplication. Current login is the native CLI login, not proof of which account an existing session uses. Email/plan are display metadata; unknown plans stay unknown. The badge shows `tier ?? plan` — `tier` is display-only (Claude "Max 5x/20x", Grok "SuperGrok Heavy").
- Home and Usage share `useAccountUsage` and `AccountUsageRows`: show each native or managed Claude/Codex/Grok account instead of the provider’s single default login. Personal windows come from native quota reports; team rows use server-reported summary capacity only (`lastRemainingPercent` + `healthReportedAt` → shown as last known). Home/Usage never lease team credentials; preserve unknown, stale and elapsed-reset states.
- Settings → Accounts (`agentAccounts`) auto-checks due personal rows (`personalUsageDue`, once per visit; Refresh forces) and each never-measured enabled team row once. The per-row team "Check usage" (user-approved 2026-09-23) is the only other path that leases a team account for measurement.
- `agentAccounts` is the dedicated Settings tab for Claude/Codex/Grok accounts and automatic fallback. Keep Git identities and existing connections in `accounts`; runtime account notices link to `agentAccounts`. Sections by owner (`AccountGroup`): "Your accounts", then one "{name} team" section per team the member can see (nothing to sign in to), and an "Open Teams" pointer when on no team. Each row carries a provider badge. Adding happens from the section: "Add account" / "Add team account" (managers only) opens the form inside that section. Per-row actions live in the `AccountMenu` (Options): Use this account (Codex/Grok; not the current login, a signed-out one, or one a teammate holds), Check usage, Move to team, Rename, Pause/Resume, Remove. Team rows show who holds them (`inUse`: "In use by Alex’s CLI", "In use by your CLI"); a current login that someone else also holds shows a both-using warning. `UsageBars` shows each reported limit (5-hour, weekly, Opus, Sonnet) separately, else the combined reading; an old reading shows its age instead of "Status unknown". Claude's "Team" plan badge reads "Team plan". Rows show "N people active" (team rows from 1, your own logins from 2). `SharedClaude` gives owners the "Shared Claude accounts" switch and lists Claude logins 2+ members are active on. The tab uses custom `ChoiceGroup` radios, never native `<select>`. Priority is backend-only now. Files: `settings/accounts/`.
- Move to team (`provider_accounts_move_to_team`, Codex/Grok, owners/managers only): an added account is uploaded then removed from Personal; the current login is shared and linked (`team_links`), so it shows under the team with the Current login badge while it stays signed in on this Mac.
- Claude accounts are personal-only: reset a Claude selection to Codex when opening a team's add form, exclude Claude team rows, and reject Claude team login requests. Native Claude is already primary; hide both “Add to switching” and “Use existing login” and never import its credentials. New personal Claude profiles use native browser sign-in. Preserve reported Sonnet/Opus/Designs/Routines windows without inventing absent limits. Runtime notices accept Claude with the same fixed-copy validation and provider/session deduplication as Codex/Grok; label them Claude and never expose native error or credential fields.
...
- Claude terminal light/dark: Claude's `"auto"` theme follows the terminal. Spawn injects `"theme":"auto"` via `--settings` (skipped for a user-chosen daltonized/ANSI/custom theme) plus `COLORFGBG` from the resolved app mode (`set_window_theme` `isLight` → `process::claude_theme`). On a flip, `ClaudeTerminalView` sends `CSI ? 997 ; 1|2 n` only if the app turned on DEC mode 2031 (`bundle.colorSchemeUpdates()`); Claude then re-reads the background via OSC 11, which xterm answers from the current theme.
