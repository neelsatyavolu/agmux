...
- Pi remote history follows the last saved parent chain, with bounded disk reads. Failed tools must retain their error rather than speculative file changes. The current provider matrix and live-test gaps are recorded in `docs/remote-control-audit-2026-09-23.md`.
- Every terminal provider has a native phone history reader (`remote/timeline.rs` + `remote/timeline/*.rs`: Droid, Kimi, Cline, Hermes, OpenCode PTY, Gemini/agy PTY; SQLite opened read-only via `sqlite_ro`). Terminal output never reaches `agent_logs` (only typed input does), so logs are a last-resort fallback. Readers bound IO (8 MiB tails; Grok keeps file line numbers as entry ids) and use deterministic ids.
- Phone Stop sends the desktop's stop key (`terminal_stop_key`: Ctrl-C for Grok, Escape for everyone else) — a second Ctrl-C quits Claude/Codex.
- Discovered Kimi/Pi/Grok terminals (`remote/discovered_terminals.rs`) reuse the desktop `list_*_sessions` commands with the sidebar filters, cached 15s. Viewing resolves a read-only synthetic thread; the first phone send claims it like a sidebar click (host row whose id is the session id, provider binding seeded) before `ensure_pty_session`.
...
- Terminal phone approvals (`remote/terminal_approvals.rs`) publish only for live Claude/Kimi PTYs from the `permission-request` hook; keys are `1` (allow once) / Esc (deny) — never `y`/`n`, which fall through to Enter = Yes. Grok (`approval_required` precedes its classifier), agy (no permission hook) and Codex MCP forms are never published. A same-call async PreToolUse must not settle the card; a different call, prompt, stop, session end, PTY exit or a desktop answer key does. Turn end/interrupt/process death call `notify_thread_requests_cleared` for chat runtimes.
...
- `provider_accounts/` owns personal Codex/Grok native OAuth homes and team lease credentials, separate from analytics. Frontend commands return metadata only. Homes share provider history/integrations but never authentication or leader sockets; never rewrite the global login to switch a session. The only exception is the explicit **Use this account** action below.
...
- `cli.rs` "Use this account" (user decision 2026-09-23): atomically rewrites the Codex/Grok CLI's own `auth.json` (never Claude, never API-key/keyring setups, refused while agmux sessions use either account). The replaced login is kept: team logins go back to the pool with their latest tokens, a saved account gets the CLI's fresher copy, anything else becomes a new personal account; a personal target is removed from Personal so one refresh-token holder remains. A team target and any current login that is already a pool account (matched by `profile::team_identity_hash`, identical to the server's) hold a `purpose: "cli"` lease; `spawn_keeper` renews it every 2 min with the CLI's refreshed tokens (or writes the pool's newer copy), and releases it when the CLI signs into something else. Each renewal also measures the CLI login (`native::refresh_usage`) and sends `remainingPercent`/`blockedUntil` + display usage, so the team row isn't stuck on an old reading. "Check usage" on a CLI-held team row goes through `cli::refresh_held` (a check lease would 409). When the CLI login's recent reading is ≤5% left, the keeper runs `check_alternatives`: the provider's other enabled, idle accounts with no reading in 5 min are refreshed (team ones via short check leases; user request 2026-09-24).
- `activity.rs` reports, every 60 s, which logins this Mac's *running agmux sessions* use (from `bindings()`: opaque `team_identity_hash`/`claude_hash`, session counts; Claude and its email only when the team owner enabled it). Never report activity outside agmux (Teams disclosure `NEVER`). `acquire` ranks personal fallbacks by teammates active on the same login (`selection::Candidate.others`, after priority); the server ranks pool allocation the same way.
