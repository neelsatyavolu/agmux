...
- **Web look:** follows agmux.dev — self-hosted Archivo (`web/fonts/`), slate tokens with light/dark from the system setting (`web/teams.css` `:root`), condensed headlines, flat ruled panels, gold only for the primary action. Use tokens (`--fill`, `--hover`, `--*-soft`/`--*-line`/`--*-text`), never raw `rgba(255,255,255,…)`, so both modes work. Mono only for machine text (handles, project keys, model IDs, commands).
...
- **Sessions vs session activity:** `sessions` per bucket = sessions active that hour (session-hours when summed; rate denominator only). The displayed "Sessions" is `sessions_started` (D1 migration 016): each top-level session once, in its first event's bucket; Codex `source.subagent` threads and Claude `subagents/` files excluded. NULL = older desktop → shown as partial, never zero. Details in `AGMUX_TEAMS.md`.
- **Grok replay dedupe:** Grok restarts `_meta.eventId` after a resume; a replay is same ID *and* identical row (`scan::grok::replay_key`, shared with the Usage panel). ID-only dedupe dropped real turns.
